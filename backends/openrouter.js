// OpenRouter backend (OpenAI-compatible). Model is a namespaced slug, e.g.
// "google/gemini-2.0-flash-exp:free" or "anthropic/claude-3.5-sonnet" — see openrouter.ai/models.

function oerr(code, msg) { const e = new Error(msg); e.code = code; return e; }

export async function generateOpenrouter({ systemPrompt, userContent, settings, signal }) {
  const key = settings.openrouterApiKey;
  if (!key) throw oerr("NO_API_KEY", "Missing OpenRouter API key");
  ...
  try {
    res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://github.com/yt-summarizer",
  ...
  if (!text) throw oerr("BAD_RESPONSE", "Empty response from OpenRouter");
  return text;
}
